...
public class AugmentedImageActivity extends AppCompatActivity implements GLSurfaceView.Renderer {
  ...
  private boolean setupAugmentedImageDatabase(Config config) {
    ...
    if (useSingleImage) {
      ...
      try (InputStream is = getAssets().open("full3.imgdb")) { // a priori ici on a l'image database , a voir si je peux la remplacer
  ...
  private Bitmap loadAugmentedImageBitmap() {
    try (InputStream is = getAssets().open("tes.imgdb")) {
